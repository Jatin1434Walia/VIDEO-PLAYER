...
for (var i = 0; i < playlistData.length; i++) {
  playlistWrapper.innerHTML += `
    <div onclick="cardClicked(${i})" id="card${i}" class="playlist-card ${
    i == 0 ? `active-card` : ""
  }">
...
function cardClicked(index) {
  ...
  $("#card" + index).addClass("active-card");

  playerWrapper.innerHTML = `
    <iframe id="video-player" src="https://player.vimeo.com/video/${videoPlaySectionData[index].vimeoId}" frameborder="0" webkitallowfullscreen mozallowfullscreen allowfullscreen></iframe>

    <div>
        <div id="video-actions">
            <p><span id="views-count">${videoPlaySectionData[index].views/1000}k</span> views</p>
        ...
        <h3 id="video-title">${videoPlaySectionData[index].title}</h3>
        <p id="video-description"> ${videoPlaySectionData[index].description}</p>
    </div>
    `;
}
